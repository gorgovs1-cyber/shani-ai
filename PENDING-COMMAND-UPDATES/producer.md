...
description: >
  ...
  contact sheet, QA) דרך marketing-engine/producer/render_visuals.py, ואחריו בניית Recording
  Studio מודרך (RECORDING-STUDIO.html) דרך marketing-engine/producer/recording_studio.py.
  לא סוכן — פקודת תזמור. לא יוצר מחקר/קופי חדש, לא משנה את החבילה, לא מקליט מסך במקום שני,
  לא מפרסם.
---
...
## שלב 3 — בניית Recording Studio מודרך (חובה כשהרינדור בשלב 2 הצליח)

**מיד אחרי** רינדור ויזואלי מוצלח בשלב 2 (כלומר `visuals/png/cover.png`, `CONTACT-SHEET.png`
ו-`visual-manifest.json` קיימים בפועל) — **באותה הרצה**, מריצים אוטומטית:

```bash
python3 marketing-engine/producer/recording_studio.py --package "marketing-engine/packages/<slug>"
```

זהו כלי מקומי-דטרמיניסטי (לא קורא ל-LLM, לא ממציא כלום) שקורא את `visual-manifest.json`,
`visual-timeline.csv`, `voiceover.txt`, `on-screen-text.csv`, `shot-list.md`,
`screen-recording-checklist.md` ו-`MANUAL-ACTIONS.md` האמיתיים ובונה קובץ יחיד:
`production/RECORDING-STUDIO.html` — צופה מודרך, שוט-אחר-שוט, עם התמונות/overlays האמיתיים
מוטמעים (base64, ללא CDN, עובד אופליין בלחיצה כפולה). זה שכבת השימושיות הסופית לשני — לא
תחליף לקבצי ה-production/ עצמם, שנשארים זמינים כרגיל.

**כשל בשלב זה** (הסקריפט נכשל / הקובץ לא נוצר) → **אינו** מכשיל את כל הרצת Producer;
מדווחים שורה קצרה ב-`MANUAL-ACTIONS.md` שה-Recording Studio לא נוצר והתיקייה `production/`
הרגילה זמינה כחלופה — לא ממציאים HTML חלופי, לא עוצרים את שאר הרצת Producer בגלל זה.

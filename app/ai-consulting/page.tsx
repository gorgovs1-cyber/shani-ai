...
const copyByLang: Record<"he" | "en", ServiceCopy> = {
  he: {
    ...
    lead:
      "ניהלתי עסק משלי 10 שנים, אז אני יודעת בדיוק איפה נשרף הזמן ואיפה AI באמת יזיז את המחט. אני ממפה את התהליכים שלכם, בונה אסטרטגיה, ומלווה בהטמעה, בעברית אמיתית, בלי באזז וורדס ובלי הבטחות ריקות.",
    ...
    sections: [
      ...
      },
      {
        ...
        p: "רוב כלי ה-AI חושבים באנגלית ומתרגמים, ולכן העברית שיוצאת נשמעת מוזרה ולא מקצועית. אני בונה סקילים, מערכות הוראות שמלמדות את ה-AI לכתוב ולחשוב בעברית אמיתית, בדיוק בשפה ובטון של העסק שלכם, כך שמה שיוצא מוכן לשימוש ונשמע כמוכם.",
      },
      {
        h: "הטמעת AI בעסק, לא נשארים לבד עם מסמך",
        p: "ייעוץ ששווה משהו לא נגמר במסמך יפה. אני מלווה אתכם בהטמעה בפועל: מגדירה את הכלים, בונה את הסקילים והאוטומציות, מדריכה את הצוות, ומוודאת שזה באמת נכנס לשגרת העבודה. המטרה היא תוצאות מדידות, לא עוד קובץ שנשכח בדרייב.",
      },
    ],
    ...
    faqItems: [
      ...
      {
        q: "כמה עולה ייעוץ AI?",
        a: "אבחון בסיסי דרך הטופס באתר הוא חינם. אבחון מעמיק עם מיפוי ותכנית עבודה מלאה הוא בתשלום שמתקזז אם מזמינים פרויקט. ליווי הטמעה מלא מתומחר לפי היקף בהצעה אישית, אחרי שנבין יחד מה בדיוק צריך.",
      },
      {

...
export default function Footer(){
 ...
 const pages=[[t.navWork,"/work"],[t.navWebsites,"/websites"],[t.navAutomations,"/automations"],[t.navConsulting,"/ai-consulting"],[t.navPricing,"/pricing"],[t.navGuides,"/guides"]];
 ...
 <div className="footer-brand"><div className="footer-brand-lockup"><Logo height={30}/><strong dir="ltr">Shani Gorgov</strong></div><p>{t.footerSeo}</p></div>
 <div className="footer-columns">
 <nav aria-label={he?"עמודי האתר":"Site pages"}><h2>{he?"באתר":"Explore"}</h2>{pages.map(([label,href])=><a key={href} href={href}>{label}</a>)}</nav>
 ...
 <nav aria-label={he?"מידע והגדרות":"Information and settings"}><h2>{he?"מידע והגדרות":"Information"}</h2>{legal.map(([label,href])=><a key={href} href={href}>{label}</a>)}<button type="button" onClick={openCookieSettings}>{he?"הגדרות עוגיות":"Cookie settings"}</button></nav>
 </div>

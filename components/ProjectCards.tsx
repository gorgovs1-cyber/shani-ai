...
<div className="project-square-copy"><h3>{item.capture==="audit"?(he?"השאלון והאוטומציה":"Questionnaire and automation"):title}</h3>{(item.capture==="mymoney"||item.capture==="beamer")&&<span className="project-progress">{he?"בעבודה":"In progress"}</span>}<p>{he?item.he:item.en}</p><a href={item.capture==="audit"?"/audit":project.liveUrl} target="_blank" rel="noopener noreferrer" className="project-site-button" aria-label={he?`כניסה לאתר ${title}, בחלון חדש`:`Visit ${title}, opens a new tab`}>{item.capture==="audit"?(he?"לשאלון":"View questionnaire"):(he?"כניסה לאתר":"Visit website")}</a></div>
 </article>;
 })}</div>;
}

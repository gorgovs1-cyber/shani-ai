...
export default function WorkGrid(){
 ...
 useEffect(()=>{
  ...
  const tick=(now:number)=>{
   ...
   if(mobile&&!paused&&!interacting&&!media.matches&&!document.documentElement.classList.contains('a11y-no-anim')&&!document.hidden&&visible&&now>holdUntil){
 ...
 <Link className="project-site-button" href="/work">{lang==="he"?"לכל הפרויקטים":"View all projects"}</Link></section>;
}
